...
solana_program::declare_id!("J66SY1YNFyXt6jat8Ek8uAUshxBY2mLrubsMRN4wggt3");
...
pub enum SwapInstruction {
    ...
    // [NEW] Close and reset program state (admin only)
    Reinitialize {
        // Admin who can manage the program
        admin: Pubkey,
        // YOT token mint address
...
fn process_swap(
    ...
    // Extract accounts (simplified for example)
    ...
    // This is critical - we require a valid YOS token account to be passed in
    let user_yos_account_data = TokenAccount::unpack(&user_yos_account.data.borrow())?;
...
/// [NEW] Close and reinitialize program state (admin only)
fn process_reinitialize(
    ...
    // Load current program state to verify admin
    if program_state_account.data_len() > 0 {
        let program_state = match ProgramState::try_from_slice(&program_state_account.data.borrow()) {
            Ok(state) => state,
            Err(_) => {
                msg!("Failed to deserialize program state, but continuing with reinitialization");
                ProgramState {
                    admin: *current_admin_account.key,
                    yot_mint: Pubkey::default(),
                    yos_mint: Pubkey::default(),
                    lp_contribution_rate: 0,
                    admin_fee_rate: 0,
                    yos_cashback_rate: 0,
                    swap_fee_rate: 0,
                    referral_rate: 0,
        ...
        // Verify current admin is the authorized admin
        if program_state.is_initialized && program_state.admin != *current_admin_account.key {
            return Err(ProgramError::InvalidAccountData);
        }
    }

    // Close and recreate program state account if needed
    if program_state_account.data_len() > 0 {
        // Transfer lamports from program account to admin (closing the account)
        let lamports = program_state_account.lamports();
        **program_state_account.lamports.borrow_mut() = 0;
        **current_admin_account.lamports.borrow_mut() += lamports;
    }

    // Create new program state account
    let rent = Rent::get()?;
    let state_size = std::mem::size_of::<ProgramState>();
    let lamports = rent.minimum_balance(state_size);
    ...
    // Initialize program state with new parameters
    let program_state = ProgramState {
        admin,
        yot_mint,
        yos_mint,
    ...
    msg!("Program successfully reinitialized");
    Ok(())
}
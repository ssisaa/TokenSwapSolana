...
fn process_initialize(
    ...
    // Create program state account with PDA
    let (pda, bump_seed) = Pubkey::find_program_address(
        &[b"program_state"],
        program_id,
    );
    
    // Verify PDA matches account
...
fn process_stake(
    ...
    if user_staking_account.data_is_empty() {
        ...
        invoke_signed(
            ...
            &[&[b"staking_account", user_account.key.as_ref(), &[bump_seed]]],
        )?;
...
pub enum StakingError {
    ...
    #[error("Not enough tokens staked")]
    InsufficientStake,
    
    #[error("Rewards below harvest threshold")]
    RewardsBelowThreshold,
}

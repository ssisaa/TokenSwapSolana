...
function loadWalletFromFile() {
  try {
    ...
    if (fs.existsSync(keypairPath)) {
      ...
      const keypairData = JSON.parse(fs.readFileSync('test-keypair.json', 'utf-8'));
      return Keypair.fromSecretKey(new Uint8Array(keypairData));
    }
  } catch (error) {
    console.error('Error loading wallet from keypair:', error);
    throw new Error('Failed to load wallet keypair. Please ensure test-keypair.json exists');
  }
}
